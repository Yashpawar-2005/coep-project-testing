...
function TeamCard({ team, dateLabel, showManage }) {
  const date = team.joinedAt || team.createdAt
  ...
  return (
    <Card className="overflow-hidden transition-all hover:shadow-md">
      ...
      <CardContent>
        <p className="text-sm text-muted-foreground">{team.description || "No description provided for this team."}</p>
      </CardContent>
      <CardFooter className="flex justify-between border-t pt-4 pb-2">
        {showManage ? (
          <Link href={`/teams/manage/${team.id}`} passHref>
          ...
          <div></div> // Empty div to maintain spacing when manage button is not shown
        )}
        <Button size="sm">
          View Team
